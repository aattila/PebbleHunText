...
{
  text_layer_set_font(textlayer, s_custom_font);
	text_layer_set_text_color(textlayer, GColorWhite);
	text_layer_set_background_color(textlayer, GColorClear);
	text_layer_set_text_alignment(textlayer, GTextAlignmentLeft);
}
...
static void main_window_load(Window *window) {
  s_custom_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_GOTHAM_LIGHT_34));
  ...
  s_msg3_layer = text_layer_create(GRect(0, 88, 144, 45));
  configureLightLayer(s_msg3_layer);

  s_msg4_layer = text_layer_create(GRect(0, 125, 144, 45));
